...
class CreateB2bMarketplaceOrdersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('b2b_marketplace_orders', function (Blueprint $table) {
            $table->increments('id');

            $table->string('status')->nullable();
            $table->boolean('is_withdrawal_requested')->default(0);
            $table->string('supplier_payout_status')->nullable();
            ...
            $table->decimal('discount_amount_invoiced', 12, 4)->default(0)->nullable();
            $table->decimal('base_discount_amount_invoiced', 12, 4)->default(0)->nullable();
            ...
            $table->unique(['supplier_id', 'order_id'],'b2b_mp_products_supplier_id_order_id_unique');
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('b2b_marketplace_orders');
    }
}
