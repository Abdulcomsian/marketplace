...
use Webkul\B2BMarketplace\Repositories\ProductRepository as B2bMarketplaceProductRepository;
...
class Cart
{
    ...
    public function __construct(
        ...
        protected B2bMarketplaceProductRepository $b2bMarketplaceProductRepository
    )
    {
        $this->initCart();
    }
    ...
    public function getItemByProduct($data)
    {
        ...
        foreach ($items as $item) {
            ...
            else if(isset($data['additional']['supplier_info']) && isset($item->additional['supplier_info'])){
                if($data['additional']['supplier_info']['product_id'] == $item->additional['supplier_info']['product_id']
                &&
                $data['additional']['supplier_info']['supplier_id'] == $item->additional['supplier_info']['supplier_id'] 
                ){
                    return $item;
                }       
            }
    ...
    public function addProduct($productId, $data)
    {
        ...
        $supplierOwnerProduct = $this->b2bMarketplaceProductRepository->findOneWhere([
            'product_id' => $productId,
            'is_owner' => 1
        ]);

        if(isset($data['supplier_info'])) {
            $data['supplier_info'] = array(
                'product_id'=> isset($data['selected_configurable_option']) ? $data['selected_configurable_option'] : $data['supplier_info']['product_id'],
                'supplier_id' => $data['supplier_info']['supplier_id'],
                'is_owner' => 0
            );
        }
        if(!isset($data['supplier_info']) && !empty($supplierOwnerProduct)){
            $data['supplier_info'] = array(
                'product_id'=> isset($data['selected_configurable_option']) ? $data['selected_configurable_option'] : $productId,
                'supplier_id' => $supplierOwnerProduct->supplier_id,
                'is_owner' => 1
            );
        }
