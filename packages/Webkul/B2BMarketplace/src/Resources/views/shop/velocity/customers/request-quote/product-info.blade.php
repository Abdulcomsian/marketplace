    ...
    <div class="rfq-product-section">
        ...
        <div class="product-head">
            <div class="add-btn">
                ...
                <button class="theme-btn" type="button" style="background: black !important;" @click="nevigate()">
                    {{ __('Back') }}
                </button>
                ...
                <div class="supplier-header-txt fieldset">
                    <div class="field-required">
                        <label class="label-text mandatory">
                            {{ __('b2b_marketplace::app.shop.supplier.account.rfq.action-category') }}
                        </label>
                    ...
                    <div class="form-group dropdown-open" style="position: relative" :class="[errors.has('product_data.name') ? 'has-error' : '']">
                        <label class="label-text mandatory">
                            {{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-name') }}
                        </label>

                        <input type="text" class="form-style dropdown-toggle"
                        placeholder="enter keyword" autocomplete="off" v-model.lazy="term" v-debounce="500"id="product_data.name" name="product_data.name" v-model="product_name" v-validate="'required'" value="{{ old('product_name') }}" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-name') }}&quot;"/>
                        <span class="control-error" v-if="errors.has('product_data.name')">@{{ errors.first('product_data.name') }}</span>

                        <label class="label-text" style="color:#21a179" v-show="product_data.product_id && product_data.product_id != null">
                            Product Price: @{{formatedPrice}}
                        </label>

                        <div class="dropdown-list bottom-left product-search-list" style="display: none;">
                                ...
                                <ul style="list-style-type: none;">
                                    <li v-if="products.length" class="table dropdown-toggle">
                                        <table id="productTable" class="productTable">
                                            <tbody>
                                                <tr v-for='(product, index) in products' @click="getProductId(product)">
                                                    <td>
                                    ...
                                    <li v-if="term.length < 3 && is_searching == false">
                    ...
                    <div class="form-group" :class="[errors.has('product_data.description') ? 'has-error' : '']">
                        <label for="label-text mandatory" class="required">
                            {{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-description') }}
                        </label>
                        <textarea type="text" class="control" id="product_data.description" name="product_data.description" v-model="product_data.description" v-validate="'required'" value="{{ old('description') }}" data-vv-as="&quot;Description&quot;"></textarea>
                    ...
                    <div class="control-group" :class="[errors.has('product_data.quantity') ? 'has-error' : '']">
                        ...
                        <input type="text" style="width:100%;" class="control product-search-box" id="product_data.quantity" name="product_data.quantity" v-model="product_data.quantity" v-validate="'required|numeric|min_value:1'" value="{{ old('quantity') }}" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-qty') }}&quot;" @keypress="isNumber($event)"/>
                        <span class="control-error" v-if="errors.has('product_data.quantity')">@{{ errors.first('product_data.quantity') }}</span>
                    </div>
