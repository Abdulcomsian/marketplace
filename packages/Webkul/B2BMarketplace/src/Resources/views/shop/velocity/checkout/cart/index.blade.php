...
@push('scripts')
                                    ...
                                    @foreach ($cart->items as $key => $item)
                                        @php
                                            ...
                                            if(isset($item['additional']['supplier_info'])){
                                                $supplier = app('Webkul\B2BMarketplace\Repositories\SupplierRepository')->find($item['additional']['supplier_info']['supplier_id']);

                                                $supplierProduct = app('Webkul\B2BMarketplace\Repositories\ProductRepository')->getMarketplaceProductByProduct($item->product->id, $supplier->id);

                                                $basesupplierProduct = isset($supplierProduct->parent_id) ? $supplierProduct->parent : $supplierProduct;

                                                if(isset($supplierProduct->is_owner) && !$supplierProduct->is_owner){
                                                
                                                    $images =  productimage()->getGalleryImages($basesupplierProduct)[0];
                                                }
                                            }
                                        ...
                                        <div class="row col-12" v-if="!isMobileDevice">
                                            ...
                                            @if (!isset($item->additional['isQuoteProduct']) && $item->product->getTypeInstance()->showQuantityBox() === true)
                                            ...
                                            @elseif(isset($item->additional['isQuoteProduct']))
                                                <div class="product-quantity col-2 no-padding">
                                                    <b2bquantity-changer
                                                        :control-name="'qty[{{$item->id}}]'"
                                                        quantity="{{ $item->quantity }}">
    ...
    <script type="text/x-template" id="b2bquantity-changer-template">

        <div :class="`quantity control-group ${errors.has(controlName) ? 'has-error' : ''}`">
            <label class="required">{{ __('shop::app.products.quantity') }}</label>
            <button type="button" class="decrease" style="background: #e5e5e5;">-</button>
            ...
            <button type="button" class="increase" style="background: #e5e5e5;">+</button>
    ...
    <script type="text/javascript">
        (() => {
            Vue.component('b2bquantity-changer', {
                template: '#b2bquantity-changer-template',
                inject: ['$validator'],
                props: {
                    controlName: {
                        type: String,
