<div class="rfq-main-container">
    <div class="account-head mb-15">
        <span class="account-heading">
            {{ __('b2b_marketplace::app.shop.supplier.account.rfq.quick-orders') }}
        </span>
            ...
            <div class="supplier-rfq">
                <div class="supplier-container">
                    <div class="supplier-request-quote">
                        {{-- <h2 class="supplier-collection-h2">Request a Quote</h2> --}}
    ...
    <script type="text/x-template" id="quick-order-template">
        ...
        @csrf()
            <div class="form-container" style="margin-top: 40px">
                <div>
                    <h3>{{ __('b2b_marketplace::app.shop.supplier.account.rfq.cart-items') }}</h3>
                    <div class="control-group">
                            ...
                            @if (count($cartItems) > 0)
                                @foreach ($cartItems as $cartItem)
                                    <tbody class="rfq-product-table-tbody">
                                        ...
                                        <td>
                                        <a href="{{route('shop.checkout.cart.remove', $cartItem->id)}}" class="btn btn-lg btn-primary" style="background: #00BF44 !important">Remove Item
                                            </a>
                                        </td>
                                        </tr>
                                    </tbody>
                        ...
                        <div class="supplier-products-row-container">
                            <div class="supplier-header-txt fieldset">
                                ...
                                <div class="control-group" :class="[errors.has('order-form.billing.name') ? 'has-error' : '']">
                                ...
                                <div class="control-group" :class="[errors.has('order-form.billing.quantity') ? 'has-error' : '']">
                                    <label class="label-text required">
                                        {{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-qty') }}
                                    </label>

                                    <input type="text" style="width:100%;" class="control product-search-box" id="billing.quantity" name="billing.quantity" v-model="billing.quantity" v-validate="'required|numeric'" value="{{ old('quantity') }}" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-qty') }}&quot;"/>
                                    <span class="control-error" v-if="errors.has('order-form.billing.quantity')">@{{ errors.first('order-form.billing.quantity') }}</span>
                                </div>

                                <button v-if="billing.product" type="submit" class="btn btn-lg btn-primary" name="button2" id="quick-order-submit-btn1">
                                    <span>
                                        {{ __('b2b_marketplace::app.shop.supplier.profile.add-to-cart') }}
                                    </span>
                                </button>

                                <button v-else="billing.product" disabled type="submit" class="btn btn-lg btn-primary" name="button2" id="quick-order-submit-btn1">
                                    <span>
                                        {{ __('b2b_marketplace::app.shop.supplier.profile.add-to-cart') }}
                                    </span>
                                </button>
                                ...
                                <modal id="configProduct" :is-open="$root.$root.modalIds.configProduct">
                                    <h3 slot='header' style="margin-top:0px !important;">
                                    ...
                                    <div slot="body">
                                        ...
                                        <div class="attributes top-20">
                                            <div v-for='(attribute, index) in childAttributes' class="attribute control-group" :class="[attribute.swatch_type, errors.has('super_attribute[' + attribute.id + ']') ? 'has-error' : '']">
                                        ...
                                        <div class="control-group" :class="[errors.has('quickOrder.formData.quantity') ? 'has-error' : '']">
                                            <label class="label-text required">
                                                {{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-qty') }}
                                            </label>

                                            <input type="text" style="width:100%;" class="control product-search-box" id="formData.quantity" name="formData.quantity" v-model="formData.quantity" v-validate="'required|numeric'" value="{{ old('quantity') }}" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.rfq.product-qty') }}&quot;"/>
                                            <span class="control-error" v-if="errors.has('quickOrder.formData.quantity')">@{{ errors.first('quickOrder.formData.quantity') }}</span>
                                        </div>

                                        <div>
                                            <button type="button" class="btn btn-lg btn-primary" @click= "getVariantProduct(selectedProductId)">
                                                <span>
                                                    {{ __('b2b_marketplace::app.shop.supplier.profile.add-to-cart') }}
                                                </span>
                                            </button>
            ...
            <div class="wk-supplier-btn-wrapper" disabled>
                <a  type="submit" class="btn btn-lg btn-primary" style="float:right !important;" name="button1" id="quick-order-submit-btn" @if (count($cartItems) == 0) disabled @else style="background: #00BF44 !important; float:right !important;" href="{{route('shop.checkout.onepage.index')}}" @endif>
                    <span>
