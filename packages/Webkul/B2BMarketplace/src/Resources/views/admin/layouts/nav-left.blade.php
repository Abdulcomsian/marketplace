...
<div class="navbar-left" v-bind:class="{'open': isMenuOpen}">
    ...
    <ul class="menubar">
        @foreach ($menu->items as $index => $menuItem)
            @if ($index == 'b2b-marketplace' && !core()->getConfigData('b2b_marketplace.settings.general.status'))
                @continue;
            @endif
            <li class="menu-item {{ $menu->getActive($menuItem) }}">
                <a class="menubar-anchor" href="{{ $menuItem['url'] }}">
                    ...
                    <span class="menu-label" style="width: 116px;">{{ trans($menuItem['name']) }}</span>
                    ...
                    @if (count($menuItem['children']) || $menuItem['key'] == 'configuration')
                        <span
                            ...
                            style="position: unset !important;"></span>
                    @endif
                </a>
