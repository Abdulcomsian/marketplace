...
@section('content')
    <div class="content">
        <form method="POST" action="" enctype="multipart/form-data" @submit.prevent="onSubmit">
            ...
            {!! view_render_event('b2b_marketplace.supplier.account.catalog.products.assign.before') !!}

            <div class="account-table-content">

                @csrf()
                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.product-detail') }}'"
                    :active="true">
                    <div slot="header">
                    ...
                    <div slot="body">
                        <div class="product-information">
                            ...
                            <div class="product-image">
                                <img
                                    src="{{ $baseProduct->base_image_url ?: bagisto_asset('themes/default/assets/images/product/meduim-product-placeholder.png') }}" />
                            </div>

                            <div class="product-details">
                                <div class="product-name">
                                    <a href="{{ url()->to('/') . '/' . $baseProduct->url_key }}"
                ...
                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.general') }}'"
                    :active="true">
                    <div slot="header">
                    ...
                    <div slot="body">
                        ...
                        <div class="control-group" :class="[errors.has('condition') ? 'has-error' : '']">
                            <label for="condition"
                                class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.product-condition') }}</label>
                            <select class="control" v-validate="'required'" id="condition" name="condition"
                                data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.product-condition') }}&quot;">
                                <option value="new">{{ __('b2b_marketplace::app.shop.supplier.account.products.new') }}
                                </option>
                                <option value="old">{{ __('b2b_marketplace::app.shop.supplier.account.products.old') }}
                                </option>
                            </select>
                        ...
                        <div class="control-group" :class="[errors.has('price') ? 'has-error' : '']">
                            <label for="price"
                                class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.price') }}</label>
                            <input type="text" v-validate="'required'" class="control" id="price" name="price"
                                value="{{ old('price') }}"
                                data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.price') }}&quot;"
                                {{ $baseProduct->type == 'configurable' ? 'disabled' : '' }} />
                            <span class="control-error" v-if="errors.has('price')">@{{ errors.first('price') }}</span>
                        </div>

                        <div class="control-group" :class="[errors.has('description') ? 'has-error' : '']">
                            <label for="description"
                                class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.description') }}</label>
                            <textarea v-validate="'required'" class="control" id="description" name="description"
                                data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.description') }}&quot;">{{ old('description') }}</textarea>
                ...
                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.images') }}'"
                    :active="true">
                    <div slot="header">
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.images') }}
                        <i class="icon expand-icon right"></i>
                    </div>
                    <div slot="body">
                ...
                @if ($baseProduct->type != 'configurable')
                    <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.inventory') }}'"
                        :active="true">
                        <div slot="header">
                            {{ __('b2b_marketplace::app.shop.supplier.account.products.inventory') }}
                            <i class="icon expand-icon right"></i>
                        </div>
                        <div slot="body">
            ...
            @if ($baseProduct->type == 'configurable')
                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.variations') }}'"
                    :active="true">
                    <div slot="header">
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.variations') }}
                        <i class="icon expand-icon right"></i>
                    </div>
                    <div slot="body">
            ...
            {!! view_render_event('b2b_marketplace.supplier.catalog.products.assign.after') !!}
...
@if ($baseProduct->type == 'configurable')
    @push('scripts')
            ...
            <td>
                <div :class="['control-group', errors.has(variantInputName + '[images][files][' + index + ']') ? 'has-error' : '']">
                    <div v-for='(image, index) in items' class="image-wrapper variant-image">
                        ...
                        <span class="icon trash-icon" style="margin-top:10% !important" @click="removeImage(image)"></span>
                    </div>
        ...
        <script>
            var super_attributes = @json(app('\Webkul\B2BMarketplace\Repositories\B2BProductRepository')->getSuperAttributes($baseProduct));
