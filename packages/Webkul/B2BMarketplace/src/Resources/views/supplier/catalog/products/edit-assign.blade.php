...
@section('page_title')
    {{ __('b2b_marketplace::app.shop.supplier.account.products.assign-add-title') }}
@endsection
            ...
            <div class="page-header">
                <div class="page-title">
                    <h1>
                        ...
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.assing-edit-title') }}
                    </h1>
                </div>

                <div class="page-action">
                    <button type="submit" class="btn btn-lg btn-primary">
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.save-btn-title') }}
                    </button>
                </div>
            </div>

            {!! view_render_event('b2b_marketplace.supplier.account.catalog.product.edit-assign.before') !!}

            <div class="account-table-content">

                @csrf()

                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.product-detail') }}'" :active="true">
                    <div slot="header">
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.product-detail') }}
                    ...
                    <div slot="body">
                        <div class="product-information">
                            ...
                            <div class="product-image">
                                <img src="{{ $product->product->base_image_url ?: bagisto_asset('themes/default/assets/images/product/meduim-product-placeholder.png') }}"/>
                            </div>
                ...
                <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.general') }}'" :active="true">
                    <div slot="header">
                        {{ __('b2b_marketplace::app.shop.supplier.account.products.general') }}
                        <i class="icon expand-icon right"></i>
                    </div>
                    <div slot="body">

                        <div class="control-group" :class="[errors.has('condition') ? 'has-error' : '']">
                            <label for="condition" class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.product-condition') }}</label>

                            <?php $selectedOption = old('condition') ?: $product->condition ?>

                            <select class="control" v-validate="'required'" id="condition" name="condition" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.product-condition') }}&quot;">
                                <option value="new" {{ $selectedOption == 'new' ? 'selected' : '' }}>{{ __('b2b_marketplace::app.shop.supplier.account.products.new') }}</option>
                                <option value="old" {{ $selectedOption == 'old' ? 'selected' : '' }}>{{ __('b2b_marketplace::app.shop.supplier.account.products.old') }}</option>
                            </select>
                            <span class="control-error" v-if="errors.has('type')">@{{ errors.first('type') }}</span>
                        </div>

                        <div class="control-group" :class="[errors.has('price') ? 'has-error' : '']">
                            <label for="price" class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.price') }}</label>
                            <input type="text" v-validate="'required'" class="control" id="price" name="price" value="{{ old('price') ?: $product->price }}" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.price') }}&quot;" {{ $product->product->type == 'configurable' ? 'disabled' : '' }}/>
                            <span class="control-error" v-if="errors.has('price')">@{{ errors.first('price') }}</span>
                        </div>

                        <div class="control-group" :class="[errors.has('description') ? 'has-error' : '']">
                            <label for="description" class="required">{{ __('b2b_marketplace::app.shop.supplier.account.products.description') }}</label>
                            <textarea v-validate="'required'" class="control" id="description" name="description" data-vv-as="&quot;{{ __('b2b_marketplace::app.shop.supplier.account.products.description') }}&quot;">{{ old('description') ?: $product->description }}</textarea>
                ...
                @include ('b2b_marketplace::supplier.catalog.products.accordians.assign-videos')

                @if ($product->product->type != 'configurable')
                    <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.inventory') }}'" :active="true">
                        <div slot="body">
                            ...
                            @foreach ($inventorySources as $inventorySource)
                                ...
                                <?php
                                    ...
                                    foreach ($product->product->inventories as $inventory) {
                                        ...
                                        $inventory->vendor_id == $product->supplier_id
                                        ) {
                                            $qty = $inventory->qty;
                                            break;
                                        }
                ...
                @if ($product->product->type == 'configurable')
                    <accordian :title="'{{ __('b2b_marketplace::app.shop.supplier.account.products.variations') }}'" :active="true">
                        <div slot="body">
            ...
            methods: {
                ...
                sourceInventoryQty (inventorySourceId) {
                    var inventories = this.variant.inventories.filter(function(inventory) {
                        return inventorySourceId === inventory.inventory_source_id && inventory.vendor_id == "{{ $product->supplier_id }}";
                    })
