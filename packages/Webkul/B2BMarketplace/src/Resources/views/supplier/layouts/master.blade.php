...
<html lang="{{ config('app.locale') }}">
    <head>
        ...
        @if(auth()->guard('supplier')->user()->addresses()->get()->first()->logo_url !== null)
        <link rel="icon" sizes="16x16" href="{{ auth()->guard('supplier')->user()->addresses()->get()->first()->logo_url ?? asset('vendor/webkul/ui/assets/images/favicon.ico') }}" />
        @else
        ...
        {!! view_render_event('bagisto.supplier.layout.head') !!}

    </head>

    <body @if (core()->getCurrentLocale() && core()->getCurrentLocale()->direction == 'rtl') class="rtl" @endif style="scroll-behavior: smooth;">
        {!! view_render_event('bagisto.supplier.layout.body.before') !!}

        <div id="app">

            <flash-wrapper ref='flashes'></flash-wrapper>

            {!! view_render_event('bagisto.supplier.layout.nav-top.before') !!}

            @include ('b2b_marketplace::supplier.layouts.nav-top')

            {!! view_render_event('bagisto.admin.layout.nav-top.after') !!}

            {!! view_render_event('bagisto.admin.layout.nav-left.before') !!}

            @include ('b2b_marketplace::supplier.layouts.nav-left')
        ...
        {!! view_render_event('bagisto.supplier.layout.body.after') !!}
